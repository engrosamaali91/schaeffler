...
def main():
    ...
    ap.add_argument("--pos-th", type=float, default=0.1, help="Position threshold [m]")
    ap.add_argument("--psi-th", type=float, default=0.0349, help="Yaw threshold [rad]")
    args = ap.parse_args()
    ...
    # real_df = add_cumdist(real_df)
    # sim_df  = add_cumdist(sim_df)

    # # choose common distance: the shorter of the two
    # s_common = min(real_df["s"].iloc[-1], sim_df["s"].iloc[-1])

    # real_df = crop_to_distance(real_df, s_common)
    # sim_df  = crop_to_distance(sim_df,  s_common)
    # print(f"[crop] matched distance = {s_common:.3f} m")
