...
def preprocess(input_csv, output_csv, goal_x, goal_y, vx_thresh, goal_tol, consec):
    ...
    seg.loc[:, ["t","x","y","yaw"]].to_csv(output_csv, index=False)
...
def main():
    ...
    ap.add_argument("--vx-th", type=float, default=0.02) # the threshold for velocity to detect if the robot is moving.
    ap.add_argument("--goal-tol", type=float, default=0.05) #tolerance for proximity to the goal.
    ap.add_argument("--consec", type=int, default=5) #number of consecutive samples needed to detect motion.
    args = ap.parse_args()
